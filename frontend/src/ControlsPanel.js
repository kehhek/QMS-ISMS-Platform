...
export default function ControlsPanel({ token }) {
  ...
  if (!token) return <p>Set a token above to view controls.</p>
  ...
  return (
    <div>
      <div style={{ marginBottom: 12 }}>
        <label style={{ marginRight: 8 }}>Framework:</label>
      ...
      <div style={{ marginTop: 12 }}>
        ...
        <span style={{ margin: '0 8px' }}>{controls.length ? `showing ${controls.length} of ${page.count}` : ''}</span>
